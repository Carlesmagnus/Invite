...
export default function App() {
  ...
  // Lock body and HTML scrolling when welcome page is active to prevent any scrolling
  useEffect(() => {
    if (!isOpen) {
...
END:VCALENDAR`;
  ...
  return (
    <div className="min-h-screen bg-brand-bg text-brand-text flex justify-center items-center font-sans antialiased relative selection:bg-brand-peach/35 overflow-x-hidden">
      ...
      {/* Background audio player (floating and persistent) */}
      <AudioPlayer url={activeSettings.musicUrl || ""} triggerPlay={triggerMusic} />

      {/* MAIN MOBILE CONTAINER - Optimized for smartphones */}
      <div className={`w-full max-w-md bg-brand-bg relative flex flex-col justify-between shadow-2xl border-x border-brand-gold/20 z-10 ${!isOpen ? "h-screen overflow-hidden" : "min-h-screen pb-10 overflow-x-hidden"}`}>
        ...
        <AnimatePresence>
          {!isOpen && (
            <motion.div
              ...
              exit={{ y: "-100%", opacity: 0 }}
              transition={{ duration: 0.8, ease: "easeInOut" }}
              className="fixed inset-y-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-brand-bg z-50 flex flex-col justify-center items-center text-center p-6 overflow-hidden select-none border-[12px] border-brand-cream rounded-[2.5rem] gap-y-5 sm:gap-y-7"
            >
              {/* Floral background inside welcome overlay */}
              <div className="absolute inset-0 opacity-[0.06] pointer-events-none select-none z-0">
                <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="space-y-1.5 z-10">
                <div className="w-12 h-12 rounded-full border border-brand-gold/30 flex items-center justify-center font-serif text-sm text-brand-gold font-bold tracking-widest bg-brand-cream/60 shadow-sm mx-auto">
                  {monogram}
                </div>
                <span className="font-sans text-[10px] font-semibold tracking-[0.2em] text-brand-gold uppercase block mt-1.5">
                  Are Cordially Invited To Celebrate
                </span>
              </div>
 
              {/* Middle Section: Couple portrait image & Names */}
              <div className="space-y-4 z-10 flex flex-col items-center">
                <div className="relative">
                  <div className="w-32 h-32 rounded-full border-2 border-brand-gold overflow-hidden p-1 bg-brand-cream shadow-lg mx-auto">
                    <img
                      src={activeSettings.coupleImageUrl}
                      alt="The Wedding Couple"
                      className="w-full h-full object-cover rounded-full"
                      referrerPolicy="no-referrer"
                    />
                ...
                <div className="space-y-1">
                  <h1 className="font-serif text-3xl sm:text-4xl font-bold tracking-tight text-brand-text leading-tight">
                    {activeSettings.brideName}
                    <span className="block font-sans font-light italic text-xl text-brand-accent my-0.5">&</span>
                    {activeSettings.groomName}
                  </h1>
 
                  <p className="font-sans text-[10px] tracking-widest text-brand-gold uppercase">
                    Save The Date • 2026
                  </p>
                </div>
              </div>
 
              {/* Bottom Section: Tap to Open Interaction Button */}
              <div className="space-y-3 w-full px-6 z-10">
                {guest && (
                  <div className="space-y-0.5">
                    <p className="font-sans text-[9px] text-brand-text/50 uppercase tracking-widest">Specially Sealed For</p>
                ...
                <motion.button
                  id="open-invitation-btn"
                  onClick={handleOpenInvitation}
                  className="w-full py-3 px-6 bg-brand-accent text-white rounded-xl text-xs sm:text-sm font-semibold tracking-wider uppercase shadow-md flex items-center justify-center gap-2 hover:bg-brand-accent/90 cursor-pointer border border-brand-peach/30"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <Sparkles className="w-4 h-4 text-brand-peach animate-spin" />
          ...
          <section className="px-6 space-y-4">
            ...
            <div className="bg-brand-cream border border-brand-gold/20 rounded-2xl p-3 shadow-sm space-y-3 overflow-hidden">
              <div className="h-32 rounded-xl overflow-hidden relative">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent flex items-end p-2.5">
                  <span className="font-serif text-xs text-white font-bold tracking-wide">
                    {activeSettings.venueName}
                  </span>
                </div>
              </div>

              <div className="space-y-1 text-xs">
                <p className="font-bold text-brand-text">{activeSettings.venueName}</p>
                <p className="text-brand-text/75 leading-relaxed text-[11px]">{activeSettings.venueAddress}</p>
              </div>

              <a
                id="maps-navigation-link"
                href={activeSettings.mapLink}
                target="_blank"
                rel="noreferrer"
                className="w-full py-2.5 bg-brand-bg hover:bg-brand-gold/10 border border-brand-gold/30 rounded-xl text-brand-gold font-semibold text-xs tracking-wide flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
              >
